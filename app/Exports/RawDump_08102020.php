...
class RawDump implements FromArray,WithHeadings
{
    ...
    */
    ...
    public function array(): array
    {
        ...
        if($ids->count()>0){
            $data = Audit::whereBetween('created_at', [$this->filterdata['start_date'], $this->filterdata['end_date']])->with(['qmsheet','audit_results.parameter_detail','audit_results.sub_parameter_detail','product','branch.city.state.region','branch.branchable','yard.branch.city.state.region','agency.branch.city.state.region','qa_qtl_detail','branchRepo.branch.city.state.region','agencyRepo.branch.city.state.region','user'])->withCount('artifact')->whereIn('id',$ids->pluck('audit_id'))->whereNotIn('id',$savedQcIds)->get();
        }
        else{
            $data = Audit::whereBetween('created_at', [$this->filterdata['start_date'], $this->filterdata['end_date']])->with(['qmsheet','audit_results.parameter_detail','audit_results.sub_parameter_detail','product','branch.city.state.region','branch.branchable','yard.branch.city.state.region','agency.branch.city.state.region','qa_qtl_detail','branchRepo.branch.city.state.region','agencyRepo.branch.city.state.region','user'])->withCount('artifact')->whereNotIn('id',$savedQcIds)->get();
        }
